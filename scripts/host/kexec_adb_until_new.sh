...
BOOT_UBUNTU_EXT4_ONCE="${BOOT_UBUNTU_EXT4_ONCE:-0}"
...
ubuntu_handoff_ok() {
    local r="$1"
    local boot_log="$OUT/round_${r}_boot_ubuntu_ext4.log"
    local phase_log="$OUT/round_${r}_ubuntu_phase_a.log"

    [ -s "$boot_log" ] || return 1
    [ -s "$phase_log" ] || return 1
    grep -qa 'boot-ubuntu-ext4: exec /phase_a_init' "$boot_log" || return 1
    grep -qa 'TARGET SOURCE.*FSTYPE' "$phase_log" || return 1
    grep -qa '/dev/loop0.*ext4' "$phase_log" || return 1
    grep -qa 'ubuntu-phase-a: watchdog feeder started pid=' "$phase_log" || return 1
    grep -qa '===== ubuntu phase A end' "$phase_log" || return 1
    return 0
}

STOCK_SERIAL="$($ADB devices | tr -d '\r' | awk 'NR>1 && $2=="device"{print $1; exit}')"
say "initrd=$INITRD dtb=${DTB_DEV:-<live>} max=$MAX lean=$LEAN_SERIAL stock=$STOCK_SERIAL panic=${PANIC_AFTER}s ubuntu_ext4_once=$BOOT_UBUNTU_EXT4_ONCE out=$OUT"
...
for r in $(seq 1 "$MAX"); do
    ...
    $ADB shell "su -c 'rm -f /sys/fs/pstore/console-ramoops-0 /sys/fs/pstore/dmesg-ramoops-*; : > /data/kexec/kxsh.log; : > /data/kexec/adbd.log; : > /data/kexec/boot_ubuntu_ext4.log; : > /data/kexec/ubuntu_phase_a.log; echo $PANIC_AFTER > /data/kexec/panic_after'" >/dev/null 2>&1

    if [ "$BOOT_UBUNTU_EXT4_ONCE" = "1" ]; then
        say "round $r: enabling Ubuntu ext4 one-shot boot"
        $ADB shell "su -c 'touch /data/kexec/boot_ubuntu_ext4.once; sync'" >/dev/null 2>&1
    fi
    ...
    if [ "$rc" = 0 ]; then
        ...
        pull_lean_logs "$r"
        [ "$BOOT_UBUNTU_EXT4_ONCE" = "1" ] && pull_ubuntu_logs "$r"
    ...
    if [ "$BOOT_UBUNTU_EXT4_ONCE" = "1" ]; then
        if ubuntu_handoff_ok "$r"; then
            say "round $r: *** UBUNTU EXT4 HANDOFF OK ***"
            say "round $r: boot_ubuntu_ext4 and phase A logs captured -> $OUT"
            echo "===== boot_ubuntu_ext4.log: handoff section ====="
            grep -aE 'begin image|mounted /dev/loop|cleaning lean|kicked watchdog|sent SIG|moving mounts|exec /phase_a_init' "$OUT/round_${r}_boot_ubuntu_ext4.log" | tail -30
            echo "===== ubuntu_phase_a.log: validation section ====="
            grep -aE 'ubuntu-phase-a: watchdog|pid1=|/dev/loop0|cgroup fs type|watchdog_feeder|phase A end' "$OUT/round_${r}_ubuntu_phase_a.log" | tail -40
            echo "==============================================="
            echo; echo "logs: $OUT"; exit 0
        fi
        say "round $r: Ubuntu ext4 handoff not validated yet; inspecting lean/pstore logs"
    fi

    if [ -s "$OUT/round_${r}_kxsh.log" ] && grep -qa 'kexec-system-init' "$OUT/round_${r}_kxsh.log"; then
        if [ "$BOOT_UBUNTU_EXT4_ONCE" = "1" ] && grep -qa 'boot_ubuntu_ext4 flag present; switching root' "$OUT/round_${r}_kxsh.log"; then
            say "round $r: reached kxsh and attempted Ubuntu handoff, but Ubuntu validation markers are missing"
            ...
            echo "==============================="
            say "round $r: Ubuntu handoff diagnosis captured -> $OUT (stopping)"
            exit 0
        fi
