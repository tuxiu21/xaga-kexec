...
This document describes implementation details. The scripts named below are
...
| Public command | Internal responsibility |
| --- | --- |
| `doctor` | locked source, input, patch and tool checks |
| `status` | stock/Ubuntu ADB state inspection |
| `prepare` | locked source fetch and patch application |
| `build` | kernel, kexec-tools, AOSP prebuilt and rootfs builders |
| `install` | optional rootfs replacement, active-slot pull and payload install |
| `boot` | one guarded stock-to-Ubuntu round |
| `test --unattended` | repeated boot, watchdog and log collection |
| `partition` | xaga GPT planning or explicitly confirmed modification |
| `backup` | offline read-only Ubuntu tar+zstd backup |

Compatibility scripts remain separate so each stage can be diagnosed, but no
stability is promised for their individual names or arguments.
...
[`REPRODUCING.md`](REPRODUCING.md) and the safe default, which does
